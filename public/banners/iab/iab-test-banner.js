    ...
    var e = {
            ...
            2023: function (e, t, n) {
                ...
                var i = n(8731),
                    ...
                    u = n(8301),
                    a = n(4226)
                t.API_KEY = '__tcfapi' // njsscan-ignore: node_api_key
                var l = (function () {
                    function e(e) {
                        if (e) {
...
function setIABTCFConsent(gvl, consents, setCookie = true) {
    ...
    if (setCookie) {
        setCookieOnBrowser(encoded, cookieName, COOKIE_LIFETIME_DAYS)
...
// });
 // IAB TCF Initialization
        ...
        const CONSENT_URL= 'http://localhost:3000/ext/cookie-banner/api/v1/consent/8c25669844d6/81582fe5-ec46-471c-9c21-6ed24b9bf589'
        /* eslint-disable */
...
function renderPrivacyBlock(label, vendorName, link) {
    if (!link || link === '#') return ''
    return `
    <div class="vendor-privacy-policy-privy-cmp-AE1VSVI8T5">
      ...
      <div class="iab-vendor-privy-cmp-AE1VSVI8T5">
        <div>${vendorName}</div>
        <a href="${link}" target="_blank" class="vendor-link-privy-cmp-AE1VSVI8T5" aria-label="Visit vendor site">
          ↗
        </a>
      </div>
    </div>
...
function renderVendorDisclosureBlocks(fullVendorList, type, gvl, toggle) {
    let vendorHtml = ''
    const purposesMap = gvl.purposes || {}
    const specialPurposesMap = gvl.specialPurposes || {}
    const featuresMap = gvl.features || {}
    const specialFeaturesMap = gvl.specialFeatures || {}
    ...
    for (const [id, vendor] of Object.entries(fullVendorList)) {
        ...
        const features = (vendor.features || []).map((f) => featuresMap[f]?.name).filter(Boolean)
        const specialFeatures = (vendor.specialFeatures || []).map((sf) => specialFeaturesMap[sf]?.name).filter(Boolean)
        const dataCategories = (vendor.dataDeclaration || []).map((dc) => dataCategoriesMap[dc]?.name).filter(Boolean)
        const deviceStorageDisclosureUrl = vendor.deviceStorageDisclosureUrl || '#'
        ...
        const legIntClaimLink = vendor.urls?.find((u) => u.legIntClaim)?.legIntClaim || '#'
        vendorHtml += `
    <div class="dropdown-wrapper-privy-cmp-AE1VSVI8T5">
      <div class="dropdown-title-privy-cmp-AE1VSVI8T5">
        <div>
      ...
      <div class="iab-dropdown-content-privy-cmp-AE1VSVI8T5 hide-privy-cmp-AE1VSVI8T5">
        ${renderPrivacyBlock('Privacy Policy', vendor.name, privacyLink)}
        ${legIntClaimLink ? renderPrivacyBlock('Legitimate Interest URL', vendor.name, legIntClaimLink) : ''}
        ${vendor.cookieMaxAgeSeconds ? showVendorRetentionPeriod(vendor.cookieMaxAgeSeconds) : ''}
        ...
        ${renderPurposeSection('Features', features, retentionMonths)}
        ${renderPurposeSection('Special Features', specialFeatures, retentionMonths)}
        ${renderDataCategories('Data Categories', dataCategories)}
        ${renderCookieDetailsTable(deviceStorageDisclosureUrl, `${type}-${id}`)}
        </div>
      </div>
          </div>
          `
...
function renderDataCategories(title, dataCategories) {
    if (!dataCategories || dataCategories.length === 0) {
        return ''
    }
    return `
    <div class="vendor-data-categories-section-privy-cmp-AE1VI8T5">
        <div class="vendor-purpose-headline-privy-cmp-AE1VSVI8T5">
            <strong>${title}</strong>
        </div>
        <div class="vendor-data-categories-list-privy-cmp-AE1VSVI8T5">
        <ul>
            ${dataCategories.map((cat) => `<li class="data-category-privy-cmp-AE1VSVI8T5">${cat}</li>`).join('')}
        </ul>
        </div>
    </div>
    `
...
function showVendorRetentionPeriod(cookieMaxAgeSeconds) {
    const cookieMaxAgeDays = Math.ceil(cookieMaxAgeSeconds / 60 / 60 / 24)
    return `
        <div class="vendor-privacy-policy-privy-cmp-AE1VSVI8T5">
          Retention Period:
          <div>
            <div>${cookieMaxAgeDays} Days</div>
          </div>
        </div>
    `
}
...
function renderVendorsFromGVL(gvl) {
    ...
    const vendorHtml = renderVendorDisclosureBlocks(fullVendorList, 'vendors', gvl, false)
    const legitimateVendorHtml = renderVendorDisclosureBlocks(legitimateVendorList, 'legitimateVendors', gvl, true)
    const vendorCount = Object.keys(fullVendorList).length
    const legitimateVendorCount = Object.keys(legitimateVendorList).length
    ...
    const html = ` <div class="sub-tab-privy-cmp-AE1VSVI8T5 sub-tab-purposes-privy-cmp-AE1VSVI8T5 active-privy-cmp-AE1VSVI8T5" data-tab-content="vendors">
        <h2 class="sub-tab-option-privy-cmp-AE1VSVI8T5 vendors-tab-privy-cmp-AE1VSVI8T5" data-tab="user-consent">User Consent (${vendorCount})</h2>
        <h2 class="sub-tab-option-privy-cmp-AE1VSVI8T5 vendors-tab-privy-cmp-AE1VSVI8T5" data-tab="legitimate-interest">Legitimate Interest (${legitimateVendorCount})</h2>
    </div>
...
function renderPurposes(gvl) {
    ...
    return `
        ...
        ${Object.values(purposes)
            .map((purpose) => renderPurposeDropdown(purpose, 'purposes', gvl, 'purposes', false, false))
            .join('')}
        </div>
        <div class="subtab-content-privy-cmp-AE1VSVI8T5 legitimate-interest-privy-cmp-AE1VSVI8T5" data-tab="legitimate-interest">
            ${legitimatePurposes
                .map((purpose) =>
                    renderPurposeDropdown(purpose, 'legitimatePurposes', gvl, 'legIntPurposes', true, false)
                )
                .join('')}
            <h3 class="iab-type-title-privy-cmp-AE1VSVI8T5"> Special Purposes </h3>
            ${Object.values(specialPurposes)
                .map((specialPurpose) =>
                    renderPurposeDropdown(specialPurpose, 'specialPurposes', gvl, 'specialPurposes', true, true)
                )
                .join('')}
        </div>
    `
...
function renderPurposeDropdown(purpose, type, gvl, consentCategoryKeyName, toggle = false, isHidden) {
    const purposeId = `${type}-${purpose.id}`
    const vendorsHTML = getVendorsLinkedToItem(purpose.id, gvl, consentCategoryKeyName)

    return dropDownWrapper(purposeId, vendorsHTML, purpose, type, toggle, isHidden, true)
}
function dropDownWrapper(purposeId, vendorsHTML = [], purpose, type, toggle = false, isHidden, vendorChips) {
    const vendorsChips = vendorChips ? renderVendorChips(vendorsHTML) : ''
    return `
    <div class="dropdown-wrapper-privy-cmp-AE1VSVI8T5"> 
      <div class="dropdown-title-privy-cmp-AE1VSVI8T5"> 
        <div>
...
function bindDropdowns(gvl) {
    document.querySelectorAll('.dropdown-title-privy-cmp-AE1VSVI8T5').forEach((title) => {
        title.addEventListener('click', () => {
            ...
            if (!content.classList.contains('hide-privy-cmp-AE1VSVI8T5')) {
                loadCookieDetailsOnDropdownOpen(content, gvl)
            }
        })
    })
}

function getVendorsLinkedToItem(itemId, gvl, consentCategoryKeyName) {
    if (!gvl || !gvl.vendors || typeof gvl.vendors !== 'object') return []

    const vendors = Object.values(gvl.vendors)
    return vendors.filter(
        (vendor) => Array.isArray(vendor[consentCategoryKeyName]) && vendor[consentCategoryKeyName].includes(itemId)
    )
}

function renderVendorChips(vendors) {
    const vendorLength = vendors.length || 'NA'
    const data = `<div class="iab-desc-title-privy-cmp-AE1VSVI8T5"><strong>Vendors(${vendorLength})</strong></div>`
...
function getTCFDataFromModel(existingConsent) {
...
function bindVendorRedirectClick() {
    const vendorLink = document.querySelector('.vendor-redirect-privy-cmp-AE1VSVI8T5')
    if (vendorLink) {
        vendorLink.addEventListener('click', function (e) {
            e.preventDefault()
            // Open the preference panel
            const customizeBtn = document.getElementById('customize-btn-privy-cmp-AE1VSVI8T5')
            if (customizeBtn) {
                customizeBtn.click()
            }
            // After panel is open, switch to the vendors tab
            const vendorsTab = document.querySelector('.tab-privy-cmp-AE1VSVI8T5[data-tab="vendors"]')
            if (vendorsTab) {
                vendorsTab.click()
            }
        })
...
function renderCookieDetailsTable(deviceStorageDisclosureUrl, vendorId) {
    if (!deviceStorageDisclosureUrl || deviceStorageDisclosureUrl === '#') {
        return ''
    }
    const tableId = `cookie-details-table-${vendorId}`

    return `
        <div class="cookie-details-section-privy-cmp-AE1VSVI8T5">
            <div class="vendor-purpose-headline-privy-cmp-AE1VSVI8T5">
                <strong>Cookie Details</strong>
            </div>
            <div id="${tableId}" 
                 class="cookie-details-placeholder-privy-cmp-AE1VSVI8T5"
                 data-url="${deviceStorageDisclosureUrl}" 
                 data-vendor-id="${vendorId}">
                <div class="cookie-details-info-privy-cmp-AE1VSVI8T5">
                    Cookie details will load here.
                </div>
            </div>
        </div>
    `
...
async function fetchCookieDetails(url) {
    try {
        if (url === '#') return null
        ...
        console.warn('Failed to fetch cookie details:', error)
        throw error
    }
}

// Load cookie details when vendor dropdown is opened
function loadCookieDetailsOnDropdownOpen(dropdownContent, gvl) {
    // Find all cookie details placeholders in this dropdown
    const cookieDetailsPlaceholders = dropdownContent.querySelectorAll(
        '.cookie-details-placeholder-privy-cmp-AE1VSVI8T5[data-url]'
    )

    cookieDetailsPlaceholders.forEach(async (placeholder) => {
        ...
        try {
            // Check cache first
            let cookieDetails
            if (cookieDetailsCache.has(url)) {
                cookieDetails = cookieDetailsCache.get(url)
            } else {
                cookieDetails = await fetchCookieDetails(url)
                // Cache the result (both success and failure)
                cookieDetailsCache.set(url, cookieDetails)
            }

            if (!cookieDetails || cookieDetails.length === 0) {
                placeholder.innerHTML =
                    '<div class="cookie-details-error-privy-cmp-AE1VSVI8T5">No cookie details available</div>'
                placeholder.setAttribute('data-loaded', 'true')
                return
            }

            // Get purposes map from global context
            const purposesMap = gvl?.purposes || {}

            const tableHtml = `
                <div class="cookie-table-wrapper-privy-cmp-AE1VSVI8T5">
                    <table class="cookie-details-table-privy-cmp-AE1VSVI8T5">
                        <thead>
                            <tr>
                                <th class="cookie-table-header-privy-cmp-AE1VSVI8T5">Cookie</th>
                                <th class="cookie-table-header-privy-cmp-AE1VSVI8T5">Domain</th>
                                <th class="cookie-table-header-privy-cmp-AE1VSVI8T5">Category</th>
                                <th class="cookie-table-header-privy-cmp-AE1VSVI8T5">Duration</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${cookieDetails.map((cookie, index) => `
                                <tr class="cookie-table-row-privy-cmp-AE1VSVI8T5">
                                    <td class="cookie-table-cell-privy-cmp-AE1VSVI8T5 w-10">${cookie.identifier}</td>
                                    <td class="cookie-table-cell-privy-cmp-AE1VSVI8T5 w-20">${stringifyDomains(cookie)}</td>
                                    <td class="cookie-table-cell-privy-cmp-AE1VSVI8T5 w-50">
                                        ${purposesAssociatedWithCookies(cookie.purposes, purposesMap, `${vendorId}-${cookie.identifier}-${index}`)}
                                    </td>
                                    <td class="cookie-table-cell-privy-cmp-AE1VSVI8T5 w-20">${formatDuration(cookie.maxAgeSeconds)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
            ...
            console.warn('Failed to load cookie details:', error)
            placeholder.innerHTML =
                '<div class="cookie-details-error-privy-cmp-AE1VSVI8T5">Failed to load cookie details</div>'
            placeholder.setAttribute('data-loaded', 'false')
        }
    })
}

function purposesAssociatedWithCookies(purposes, purposesMap, cookieId) {
    if (!purposes || purposes.length === 0) return ''

    const firstPurpose = purposesMap[purposes[0]]?.name || 'Unknown Purpose'
    const remainingCount = purposes.length - 1

    if (remainingCount === 0) {
        return `<div class="cookie-purpose-privy-cmp-AE1VSVI8T5">${firstPurpose}</div>`
    }

    const dropdownId = `cookie-purposes-${cookieId}`
    const toggleId = `toggle-${cookieId}`

    return `
        <div class="cookie-purposes-container-privy-cmp-AE1VSVI8T5">
            <div class="cookie-purposes-header-privy-cmp-AE1VSVI8T5">
                <div class="cookie-purpose-privy-cmp-AE1VSVI8T5">${firstPurpose}</div>
                <div class="cookie-purposes-toggle-privy-cmp-AE1VSVI8T5" onclick="toggleCookiePurposes('${dropdownId}', '${toggleId}')">
                    <span id="${toggleId}" class="cookie-toggle-text-privy-cmp-AE1VSVI8T5">+${remainingCount}</span>
                </div>
            </div>
            <div id="${dropdownId}" class="" style="display: none;">
                ${purposes.slice(1).map(purposeId => 
        `<div class="cookie-purpose-privy-cmp-AE1VSVI8T5">${purposesMap[purposeId]?.name || 'Unknown Purpose'}</div>`
    ).join('')}
            </div>
        </div>
    `
...
// function to toggle cookie purposes dropdown inside vendor details
function toggleCookiePurposes(dropdownId, toggleId) {
...
function formatDuration(maxAgeSeconds) {
    const days = Math.floor(maxAgeSeconds / (24 * 60 * 60))
...
function stringifyDomains(cookie) {
    if (Array.isArray(cookie.domains)) {
        return cookie.domains.join(', ')
    } else if (Array.isArray(cookie.domain)) {
        return cookie.domain.join(', ')
    ...
    return ''
        ...
        function sendEventDetails(dataFiduciaryId, bannerId, type) {
                    fetch(`http://localhost:3000/ext/cookie-banner/api/v1/user-interaction/events/8c25669844d6/81582fe5-ec46-471c-9c21-6ed24b9bf589`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
...
function toggleConsentUIState() {
 ...
 //Common 
    ...
    <div class="idfy-banner-privy-cmp-AE1VSVI8T5" id="banner-home-privy-cmp-AE1VSVI8T5">
        <div class="banner-content-privy-cmp-AE1VSVI8T5">
            <h2 class="banner-heading-privy-cmp-AE1VSVI8T5">Cookie Notice</h2>
            <div>
                <p class="description-privy-cmp-AE1VSVI8T5 ${template.bannerType}-desc-privy-cmp-AE1VSVI8T5" >We use cookies and similar technologies to store and access personal information on your device, such as IP addresses and unique identifiers. This helps us keep the website secure and reliable, customise your experience, evaluate site performance, and show relevant offers and ads, as explained in our privacy and cookie policies. Some of our partners process personal data based on legitimate interests. You have the right to object to this processing at any time. By clicking “Customize,” you can see a list of our <a class="vendor-redirect-privy-cmp-AE1VSVI8T5">${Object.keys(gvl.vendors).length} partners</a> and the purposes for which your consent is needed. You don’t have to give consent to access the site’s content, but without it, some personalized content and advertising may not be available. Your preferences will only apply to this site. You can withdraw or modify your consent at any time. To do so, click the floating cookie icon that appears on the screen after your first consent. Clicking the icon will reopen the banner where you can review, manage, and save your consent preferences.</p>
                <div class="userconsent-privy-cmp-AE1VSVI8T5 screen0-scroll-privy-cmp-AE1VSVI8T5" data-tab="user-consent">
                    ${dropDownWrapper('screen-1-purposes', [], gvl['purposes']['1'], 'purposes', false, true, false)}
                    ${dropDownWrapper('screen-1-stack', [], gvl['stacks']['41'], 'purposes', false, true, false)}
                    ${dropDownWrapper('screen-1-special-features', [], gvl['stacks']['1'], 'specialFeatures', false, true, false)}
                </div>
                <div class="${template.bannerType}-button-container-privy-cmp-AE1VSVI8T5">
                    <button onclick="submitConsentHandler('all')" class="${template.bannerType}-button-privy-cmp-AE1VSVI8T5">${template.buttonsText.acceptAll}</button>
    ...
    <div class="content-privy-cmp-AE1VSVI8T5">
        <p class="box-description-privy-cmp-AE1VSVI8T5">Please review and manage your cookie preferences by enabling or disabling the consent toggles for each Purpose listed below. The use of these cookies will depend on your consent; however, in some cases, the personal data collected through these cookies may be processed based on a legitimate interest by us or our third-party partners. You can find more information about these legitimate interests, including links to the applicable privacy policies, in the “Legitimate Interest” section. You also have the right to object to such data processing. Your consent preferences are stored in cookie named euconsent and is valid for ${COOKIE_LIFETIME_DAYS} days.</p>
    ...
    initializeTabs(gvl)
    setupBannerBasedOnConsentIABTCF(`8c25669844d6`, `81582fe5-ec46-471c-9c21-6ed24b9bf589`)
        bindVendorRedirectClick()
        }
        
        document.addEventListener("click", function (event) {
            if (event.target.id === "customize-btn-privy-cmp-AE1VSVI8T5") {
                sendEventDetails(`8c25669844d6`, `81582fe5-ec46-471c-9c21-6ed24b9bf589`, 'CustomizeCookiesView');
            }

            if (event.target.id === "preference-privy-cmp") {
                toggleBanner('preference')
                sendEventDetails(`8c25669844d6`, `81582fe5-ec46-471c-9c21-6ed24b9bf589`, 'PreferenceCenter');
            }
}) //Common 
         //Common